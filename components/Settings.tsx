...
const Settings: React.FC = () => {
  ...
  const providerEnvHint: Record<string, string> = {
      auto: 'Local routing only (no API keys used)',
      local: 'Offline only'
  };
  ...
  return (
    <div className="p-4 md:p-8 pt-16 md:pt-8 h-full overflow-y-auto max-w-4xl mx-auto space-y-8 scrollbar-thin scrollbar-thumb-slate-700 pb-32">
      ...
      <div className="bg-slate-900/50 border border-slate-800 rounded-3xl p-8 space-y-4">
          ...
          <p className="text-slate-400 text-sm">Cognition now runs entirely on-device. Choose between adaptive local routing or a strict offline-only mode.</p>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              ...
              >
                  ...
                  <div className="font-bold text-slate-100">Adaptive Local</div>
                  <span className="text-[10px] text-indigo-300 uppercase">Default</span>
                </div>
                  <p className="text-xs text-slate-400 mt-1">Stay offline by default and optimize local heuristics without ever calling external APIs.</p>
              </button>
